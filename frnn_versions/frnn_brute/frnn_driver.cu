...
__global__ void frnn_brute_kernel(
        const scalar_t* pts,
        const int    pts_size0,
        const int    pts_size1,
    ...
    for (int row_a = row_start + threadIdx.x; row_a < row_end; row_a += blockDim.x)
    {
        for (int row_b = row_a + 1; row_b < row_end; row_b++)
        {
            ...
            float ay = float( pts[row_a * pts_size1 + 0] );
            float ax = float( pts[row_a * pts_size1 + 1] );
            float as = float( pts[row_a * pts_size1 + 4] );

            float by = float( pts[row_b * pts_size1 + 0] );
            float bx = float( pts[row_b * pts_size1 + 1] );
            float bs = float( pts[row_b * pts_size1 + 4] );
...
template <typename scalar_t>
__global__ void frnn_brute_kernel_2(
        const scalar_t* pts,
        const int       pts_size0,
        const int       pts_size1,
...
//    frnn_brute_kernel<<<blocks, threads>>>(
...
//    ); })); CudaCheckError();
    ...
    AT_DISPATCH_FLOATING_TYPES_AND(torch::ScalarType::Half, pts.scalar_type(), "frnn_brute_kernel", ([&] {
    frnn_brute_kernel_2<<<blocks, threads>>>(
            pts.data_ptr<scalar_t>(),
            pts.size(0),
            pts.size(1),
