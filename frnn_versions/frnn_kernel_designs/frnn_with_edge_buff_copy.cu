...
__global__ void frnn_cuda_forward_kernel(
    ...
    */
    ...
    __shared__ int max_atomic_i_buff;
    ...
    if (threadIdx.x == 0) {
        ...
        i_bin_a = blockIdx.x;
        /*
        i_edges is a global memory variable that is 
        atomicadded when a block's edge buffer is full..
        */
        // i_edges = blockIdx.x * n_max_edges_per_bin * edge_size;
        atomic_i_buff = 0;
        max_atomic_i_buff = (n_edges_buff - 1) * edge_size;
    ...
    for (int idx_bin_b=0; idx_bin_b<n_max_neighbors; idx_bin_b++) {
        ...
        ////////////////////////
        if (threadIdx.x == 1) {
            // TODO: GO OVER THIS WAIT SECTION AGAIN...
            while (threads_left_counter != 0) {
                // wait:
                while ((atomic_i_buff / edge_size) < n_edges_buff &&
                    atomic_buff_counter < n_edges_buff * edge_size &&
                    threads_left_counter > 0) {
        ...
        /*
        changing this ^^^ actually th 0 and th 1 will do this step
        */
        int ia;
        for (int ja = 0; ja < n_max_pts_bin; ja++) {
            ...
            for (int ib = 0; ib < n_max_pts_bin; ib++) {
                ...
                */
                ...
                if (bin_a_ids[ia] == bin_b_ids[ib]) {continue;}

                edge_buff[i_buff + 0] = bin_a_ids[ia];
                edge_buff[i_buff + 1] = bin_b_ids[ib];

                int e_buffer_size = atomicAdd(&atomic_buff_counter, edge_size) + edge_size;

                if (e_buffer_size == n_edges_buff * edge_size) {
                    printf("emptying buffer . . .");
                    // empty the buffer and put it in the edges array
                    /*
                    TODO: FIX atomic_buff_counter to be the index not actual
                    number of edges (so number of edges / edge_size)
                    */
                    // TODO: CHECK THAT THIS IS NOT OFF BY "EDGE_SIZE"
                    i_edges_start = atomicAdd(&i_edges, e_buffer_size);

                    int e_stop = e_buffer_size;
                    if (i_edges_start + e_buffer_size - edge_size > max_i_edges) {
                        e_stop = max_i_edges - i_edges_start;
                        printf("ran out of space in edges output tensor . . .")
                    }

                    for (int ie = 0; ie < e_stop; ie ++) {
                        edges[i_edges_start + ie] = edge_buff[ie];
                    }

                    atomic_i_buff = 0;
                    atomic_buff_counter = 0;
                    
                    printf("emptied buffer in loop . . .");
                }
            }
        }
        ...
        //     assert(atomic_i_buff == atomic_buff_counter * edge_size);
            ...
            printf("emptying buffer out of loop . . .");

            /*
                    TODO: FIX atomic_buff_counter to be the index not actual
                    number of edges (so number of edges / edge_size)
            */
            i_edges_start = atomicAdd(&i_edges, atomic_buff_counter);
...
//   int n_max_edges_per_bin = edges.size(1);
  ...
  int i_edges = 0;
  const int max_i_edges = edges.size(0) * edges.size(1);
