...
__global__ void frnn_brute_lin_kernel(
    ...
    for (int row_a = threadIdx.x; row_a < row_end-1; row_a += blockDim.x)
    {
        for (int row_b = row_a + 1; row_b < row_end; row_b++)
        {
            ...
            // frnn_1
//            bool check = (dist < ( lin_radius * ((as+bs)/2) )) && (fabsf(logf(as) - logf(bs)) < scale_radius);

            // frnn_2
//            bool check = (dist < ( lin_radius * ((as+bs)/2) ));

            // frnn_3
//            bool check = (dist < ( lin_radius * pow( ((as+bs)/2), 2) ));

            // frnn_5
//            bool check = (dist < ( lin_radius * sqrt(as*bs) ) );

            // frnn_10
//            bool check = (dist < ( lin_radius * pow((as+bs)/2,3) ) );

            // frnn_11
//            bool check = (dist < ( lin_radius * ( (pow(as,2) + pow(bs,2)) / 2 ) ) );
