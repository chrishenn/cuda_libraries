...
// load a bin's pts, where we are interested in [y, x, z, angle, scale, group_id] at columns
// [0,1,2,4,5] in pts. A bin starts at bin_start in bin_ptids and includes contiguous
// rows, of number bin_size.
                        ...
                        const int bin_stride)
{
    for (int i = threadIdx.y * blockDim.x + threadIdx.x; i < bin_size; i += blockDim.x * blockDim.y)
    {
        ...
        bin[i * bin_stride + 5] = float( pts[pt_id * pt_size1 + 5] );
    }
}
        ...
        int bin_stride, bool samebin )
{
    ...
    for (int ia = thid; ia < bin_Awidth; ia += block_threads)
    {
        ...
        float a_groupid =  bin_a[ia * bin_stride + 5];
        ...
        for (int ib = 0; ib < bin_Bwidth; ib++)
        {
            ...
            float b_groupid =  bin_b[ib * bin_stride + 5];
            ...
            // a_ptid and b_ptid are in different object-groups groups
            bool check0 = fabsf(a_groupid - b_groupid) > 0.5;

            // a_ptid is not b_ptid
            bool check1 = fabsf(ptid_diff) > 0.2;

            // if same bin a and b, no redundant edges written
            bool check2 = (!samebin) || (ptid_diff > 0);

            // frnn criteria in linear, scale space
            bool check3 = dist < ( radius * sqrtf(as*bs) ) && fabsf(logf(as) - logf(bs)) < scale_radius;
...
template <typename scalar_t>
__global__ void frnn_bipart_main_kernel(
...
void frnn_bipart_kern::frnn_bipart_kern_launch(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(pts.scalar_type(), "frnn_bipart_main_kernel", ([&] {
        frnn_bipart_main_kernel<<<blocks, threads, shared>>>(
