...
/** 
cpu entry point 
Guarantee that max location nouse < 2 pixels
**/
std::vector<torch::Tensor> grid_bin_cuda_call(
    ...
    if (shared > 49152){
        fprintf (stderr, "ERROR GRID_BIN: ATTEMPTED TO ALLOCATE TOO MUCH SHARED MEMORY FOR YOUR DEVICE ARCH; DECREASE OBJECT-DENSITY\n");
