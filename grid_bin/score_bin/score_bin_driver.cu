...
)
{
    ...
    for (int loc_i = threadIdx.x; loc_i < n_bins; loc_i += blockDim.x)
    {
        ...
        int keep_num = int( float(bin_count) * keep_frac );
...
std::vector<torch::Tensor> score_bin_cuda_call(
    torch::Tensor counts,
    ...
    if (shared > 49152){
        fprintf (stderr, "ERROR SCORE_BIN: ATTEMPTED TO ALLOCATE TOO MUCH SHARED MEMORY FOR YOUR DEVICE ARCH; DECREASE OBJECT-DENSITY\n");
        fprintf (stderr, "attemped: %li bytes; max supported: 49152 bytes\n", shared);
        exit(EXIT_FAILURE);
    }
    
    ring_kern<<<blocks, threads, shared>>>(
        counts.data<int>(),
        offsets.data<int>(),
        binid.data<int>(),   
        scores.data<float>(),                
